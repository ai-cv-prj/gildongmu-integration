...
레포를 clone한 것만으로 준비되지 않습니다. 필요한 파일과 입출력 폴더를 별도로 준비하세요.
...
가중치를 복사하지 않고 기존 위치를 설정이나 실행 옵션으로 지정해도 됩니다.
Ultralytics는 첫 실행 시 사용자 설정 폴더(일반적인 Linux 환경에서는 `~/.config/Ultralytics`)를
자동 생성할 수 있습니다. 이는 프로젝트의 영상 출력 폴더와 별개입니다.
